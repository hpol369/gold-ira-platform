        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-[#121423] border border-slate-700 rounded-lg px-4 py-3 text-[#F6F4EF] focus:outline-none focus:border-purple-500"
                >
            ...
            <div className="bg-[#0C0D18] backdrop-blur-sm rounded-xl p-6 border border-[#2A2D42]">
              ...
              <div className="space-y-2">
                {[
                  ...
                  >
                    <div className="font-semibold text-[#F6F4EF]">{option.label}</div>
                    <div className="text-sm text-[#A8A39A]">{option.desc}</div>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="bg-[#0C0D18] backdrop-blur-sm rounded-xl p-6 border border-[#2A2D42]">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-sm text-[#A8A39A] mb-2 block">
                  Your Marginal Tax Bracket: <span className="text-[#F6F4EF] font-semibold">{yourTaxBracket}%</span>
                </label>
                <input
                  type="range"
                  min="10"
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-[#121423] border border-slate-700 rounded-lg px-4 py-3 text-[#F6F4EF] focus:outline-none focus:border-green-500"
                >
                ...
                {calculations.stateInfo && (
                  <p className="text-xs text-[#D4A94E] mt-2">
              ...
              <div>
                <label className="text-sm text-[#A8A39A] mb-2 block">
                  Your Current Age: <span className="text-[#F6F4EF] font-semibold">{currentAge}</span>
                </label>
                <input
                  type="range"
                  min="25"
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-br from-purple-500/20 to-pink-500/20 backdrop-blur-sm rounded-xl p-6 border border-[rgba(197,149,46,0.32)]">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-[#A8A39A]">Gross Inheritance</span>
                <span className="text-[#F6F4EF]">{formatCurrency(inheritanceAmount)}</span>
              </div>
              {calculations.immediatesTax > 0 && (
                <div className="flex justify-between">
                  <span className="text-[#A8A39A]">Income Tax (over 10 years)</span>
                  <span className="text-red-600">-{formatCurrency(calculations.immediatesTax)}</span>
                </div>
              )}
              {calculations.stateInheritanceTax > 0 && (
                <div className="flex justify-between">
                  <span className="text-[#A8A39A]">State Inheritance Tax</span>
                  <span className="text-red-600">-{formatCurrency(calculations.stateInheritanceTax)}</span>
                </div>
              )}
              {calculations.stepUpBenefit > 0 && (
                <div className="flex justify-between pt-2 border-t border-[#2A2D42]">
                  <span className="text-[#A8A39A]">Step-Up in Basis Benefit</span>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[#2A2D42]">
                      ...
                      <th className="py-2 text-right text-[#A8A39A]">Distribution</th>
                      <th className="py-2 text-right text-[#A8A39A]">Tax ({yourTaxBracket}%)</th>
                      <th className="py-2 text-right text-[#A8A39A]">After Tax</th>
                    </tr>
                  </thead>
                  <tbody>
                    {calculations.iraDistributions.slice(0, 5).map((dist) => (
                      <tr key={dist.year} className={dist.year % 2 === 0 ? "bg-[#0C0D18]" : ""}>
                        <td className="py-2 text-[#D0CCC4]">{dist.year}</td>
                        <td className="py-2 text-righttext-[#F6F4EF]">{formatCurrency(dist.rmd)}</td>
                    ...
                    <tr className="border-t border-[#2A2D42] font-semibold">
                      <td className="py-2text-[#F6F4EF]">Total (10 yr)</td>
                      <td className="py-2 text-righttext-[#F6F4EF]">{formatCurrency(inheritanceAmount)}</td>
          ...
          {assetType === "roth_ira" && (
            <div className="bg-[rgba(46,139,87,0.08)]0/10 backdrop-blur-sm rounded-xl p-6 border border-green-500/30">
              ...
              <div className="mt-4 p-3 bg-[#0C0D18] rounded-lg">
                <div className="flex justify-between">
                  <span className="text-[#A8A39A]">Tax Savings vs Traditional IRA</span>
          ...
          {(assetType === "property" || assetType === "stocks") && calculations.stepUpBenefit > 0 && (
            <div className="bg-[rgba(46,139,87,0.08)]0/10 backdrop-blur-sm rounded-xl p-6 border border-green-500/30">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-[#A8A39A]">Original Cost Basis</span>
                  <span className="text-[#F6F4EF]">{formatCurrency(originalCostBasis)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#A8A39A]">Value at Inheritance</span>
                  <span className="text-[#F6F4EF]">{formatCurrency(inheritanceAmount)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#A8A39A]">Gain Avoided</span>
                  <span className="text-[#F6F4EF]">{formatCurrency(inheritanceAmount - originalCostBasis)}</span>
                </div>
                <div className="flex justify-between pt-2 border-t border-[#2A2D42]">
                  <span className="text-[#D0CCC4] font-semibold">Tax Savings (at {yourTaxBracket}%)</span>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[#2A2D42]">
                    ...
                    <th className="py-2 text-right text-[#D4A94E]">Gold IRA (6%)</th>
                  </tr>
                </thead>
                <tbody>
                  {calculations.projections.filter((_, i) => i === 0 || i === 5 || i === 10 || i === 20).map((proj, idx) => (
                    <tr key={proj.year} className={idx % 2 === 0 ? "bg-[#0C0D18]" : ""}>
                      ...
                      <td className="py-2 text-right text-[#D4A94E]">{formatCurrency(proj.goldIra)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div className="max-w-4xl mx-auto mt-16">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="bg-[#0C0D18] backdrop-blur-sm rounded-xl p-6 border border-[#2A2D42]">
            ...
            <h3 className="text-lg font-semibold text-[#F6F4EF] mb-2">3. Diversify Wisely</h3>
            <p className="text-[#D0CCC4] text-sm">
      ...
      <div className="max-w-4xl mx-auto mt-16">
        <div className="bg-[rgba(220,38,38,0.06)] backdrop-blur-sm rounded-xl p-8 border border-[rgba(197,149,46,0.32)]">
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            ...
            <div>
              <div className="text-[#D4A94E] font-semibold mb-2">Tax-Advantaged Growth</div>
              <p className="text-[#D0CCC4] text-sm">
        ...
        <div className="prose max-w-none">
          ...
          <div className="bg-[#0C0D18] backdrop-blur-sm rounded-xl p-6 border border-[#2A2D42] mb-6">
            ...
            <p className="text-[#A8A39A] mb-4">
              <strong className="text-[#F6F4EF]">Estate Tax:</strong> Federal tax paid by the estate before distribution.
            ...
            <p className="text-[#A8A39A]">
              <strong className="text-[#F6F4EF]">Inheritance Tax:</strong> State tax paid by the beneficiary after
          ...
          <div className="bg-[#0C0D18] backdrop-blur-sm rounded-xl p-6 border border-[#2A2D42] mb-6">
            ...
            <ul className="space-y-2 text-[#A8A39A]">
              <li>- <strong className="text-[#F6F4EF]">Spouse beneficiaries:</strong> Can roll over to their own IRA and follow normal RMD rules</li>
              <li>- <strong className="text-[#F6F4EF]">Non-spouse beneficiaries:</strong> Must withdraw all funds within 10 years (all distributions are taxable income)</li>
              <li>- <strong className="text-[#F6F4EF]">Eligible Designated Beneficiaries:</strong> Minor children, disabled/chronically ill, and those within 10 years of the decedent&apos;s age can still stretch</li>
            </ul>
          </div>
          ...
          <div className="bg-[#0C0D18] backdrop-blur-sm rounded-xl p-6 border border-[#2A2D42]">
            ...
            <div className="bg-[#121423] rounded-lg p-4">
              <p className="text-sm text-[#A8A39A]">
                <strong className="text-[#F6F4EF]">Example:</strong> Parent bought stock for $50,000, now worth $500,000.
