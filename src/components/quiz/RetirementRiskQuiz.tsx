  ...
  return (
    <div className="mb-8">
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-semibold text-[#F6F4EF]">
          Question {current + 1} of {total}
        </span>
        <span className="text-sm text-[#A8A39A]">{Math.round(progress)}%</span>
      </div>
      <div className="h-2.5 bg-[#121423] rounded-full overflow-hidden border border-[#2A2D42]">
        <motion.div
  ...
  return (
    <motion.button
      ...
      className={cn(
        ...
        isSelected
          ? "border-[#000080] bg-[rgba(197,149,46,0.06)] text-[#F6F4EF] shadow-md"
          : "border-[#2A2D42] bg-white text-[#D0CCC4] hover:border-[#B22234]/40 hover:bg-[#121423] hover:shadow-sm"
      )}
    >
      <div className="flex items-center gap-3">
        <span
          className={cn(
            ...
            isSelected
              ? "border-[#000080] bg-[rgba(197,149,46,0.1)] text-white"
              : "border-[#3F4460] text-[#A8A39A]"
          )}
        >
...
function ResultsScreen({
  ...
  return (
    ...
    >
      {/* Score display */}
      <div className="mb-8">
        <div className="inline-flex items-center justify-center w-28 h-28 rounded-full border-4 border-[#2A2D42] bg-[#161828] shadow-lg mb-4">
          <div>
            <div className="text-4xl font-bold text-[#F6F4EF]">{score}</div>
            <div className="text-xs text-[#A8A39A] font-medium">out of 30</div>
          </div>
        </div>
      </div>
      ...
      <div className={cn("rounded-xl p-6 border mb-6 text-left", colors.bg, colors.border)}>
        ...
        <p className="text-[#D0CCC4] leading-relaxed mb-4">{category.description}</p>
        <h4 className={cn("text-base font-bold mb-2", colors.text)}>
          Our Recommendation
        </h4>
        <p className="text-[#D0CCC4] leading-relaxed">{category.recommendation}</p>
      </div>
      ...
      <div className="space-y-4">
        {category.ctaType === "gold-ira-guide" && (
          <>
            <Link
              ...
              className="inline-flex items-center gap-2 px-6 py-3 bg-[rgba(197,149,46,0.1)] text-white font-bold rounded-xl hover:bg-[rgba(197,149,46,0.1)]/90 transition-colors"
            >
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap justify-center gap-3 mt-4">
              <Link
                ...
                className="text-sm text-[#D4A94E] font-medium hover:underline"
              >
        ...
        {category.ctaType === "foundations" && (
          <div className="space-y-3">
            <div className="bg-[#0C0D18] border border-[#2A2D42] rounded-xl p-6 text-left">
              <h4 className="font-bold text-[#F6F4EF] mb-3 flex items-center gap-2">
                <GraduationCap className="h-5 w-5" />
                Build Your Foundation First
              </h4>
              <ol className="space-y-2 text-sm text-[#D0CCC4]">
                <li className="flex items-start gap-2">
                  <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[rgba(197,149,46,0.1)] text-white text-xs flex items-center justify-center font-bold mt-0.5">1</span>
                  Build a 3-month emergency fund
                </li>
                <li className="flex items-start gap-2">
                  <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[rgba(197,149,46,0.1)] text-white text-xs flex items-center justify-center font-bold mt-0.5">2</span>
                  Pay down high-interest debt above 8% APR
                </li>
                <li className="flex items-start gap-2">
                  <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[rgba(197,149,46,0.1)] text-white text-xs flex items-center justify-center font-bold mt-0.5">3</span>
                  Maximize your employer 401(k) match
                </li>
                <li className="flex items-start gap-2">
                  <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[rgba(197,149,46,0.1)] text-white text-xs flex items-center justify-center font-bold mt-0.5">4</span>
                  <span>
                    Learn the basics &mdash;{" "}
                    <Link href="/learn" className="text-[#D4A94E] font-medium hover:underline">
                      start here
                    </Link>
                  </span>
                </li>
      ...
      <button
        onClick={onRestart}
        className="mt-8 inline-flex items-center gap-2 text-sm text-[#A8A39A] hover:text-[#F6F4EF] transition-colors font-medium"
      >
...
export function RetirementRiskQuiz({ onComplete }: RetirementRiskQuizProps) {
  ...
  if (quizState === "results") {
    ...
    return (
      <div className="bg-[#161828] rounded-2xl border border-[#2A2D42] shadow-lg p-6 md:p-10 max-w-2xl mx-auto">
        <ResultsScreen
          score={score}
          category={category}
  ...
  return (
    <div className="bg-[#161828] rounded-2xl border border-[#2A2D42] shadow-lg p-6 md:p-10 max-w-2xl mx-auto">
      ...
      <AnimatePresence mode="wait" custom={direction}>
        ...
        >
          {/* Question */}
          <h2 className="text-xl md:text-2xl font-bold text-[#F6F4EF] mb-6 leading-snug">
            {question.question}
          </h2>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-[#2A2D42]">
        <button
          ...
          className={cn(
            ...
            isFirstQuestion
              ? "text-[#3F4460] cursor-not-allowed"
              : "text-[#D0CCC4] hover:bg-[#121423] hover:text-[#F6F4EF]"
          )}
        >
        ...
        <button
          ...
          className={cn(
            ...
            hasAnswered
              ? "bg-[#DC2626] text-white hover:bg-[#DC2626]/90 shadow-md hover:shadow-lg"
              : "bg-[#121423] text-[#A8A39A] cursor-not-allowed"
          )}
        >
