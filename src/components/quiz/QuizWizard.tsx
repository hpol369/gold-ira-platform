...
export function QuizWizard() {
    ...
    const renderIntro = () => (
        <div className="text-center space-y-8 py-12">
            <div className="inline-flex items-center gap-2 rounded-full bg-[rgba(220,38,38,0.1)] px-4 py-1.5 text-sm font-semibold text-[#D4A94E] mb-4">
                <AlertTriangle className="h-4 w-4" />
                Are You Exposed?
            </div>
            <h2 className="text-3xl md:text-5xl font-serif font-bold text-[#F6F4EF]">{steps[0].title}</h2>
            <p className="text-xl text-[#D0CCC4] max-w-xl mx-auto leading-relaxed">{steps[0].subtitle}</p>
            ...
            <p className="text-sm text-[#A8A39A]">Takes less than 60 seconds. No SSN required.</p>
        </div>
    );

    const renderQuestion = () => (
        <div className="max-w-2xl mx-auto py-8">
            <h2 className="text-2xl md:text-3xl font-serif font-bold text-[#F6F4EF] mb-2 text-center">
                {currentStep.question}
            </h2>
            {currentStep.helper && (
                <p className="text-center text-[#A8A39A] mb-8 text-sm">{currentStep.helper}</p>
            )}
            ...
            <div className="grid grid-cols-1 gap-4 mt-8">
                {currentStep.options?.map((option) => (
                    <button
                        ...
                        className="group relative flex items-center justify-between p-6 text-left rounded-xl border-2 border-[#2A2D42] hover:border-[#B22234]/50 hover:bg-[#121423] transition-all duration-200 bg-white"
                    >
                        <span className="text-lg font-medium text-[#F6F4EF] group-hover:text-[#D4A94E]">
                            {option.label}
                        </span>
                        <div className="h-6 w-6 rounded-full border-2 border-[#2A2D42] group-hover:border-[#B22234] flex items-center justify-center">
                            <div className="h-3 w-3 rounded-full bg-[#DC2626] opacity-0 group-hover:opacity-100 transition-opacity" />
                        </div>
                    </button>
                ))}
            </div>
    ...
    const renderAnalyzing = () => (
        <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="w-16 h-16 border-4 border-[#2A2D42] border-t-[#B22234] rounded-full animate-spin mb-8" />
            <h3 className="text-2xl font-serif font-bold text-[#F6F4EF] mb-2">Analyzing your profile...</h3>
            <p className="text-[#A8A39A]">Comparing your portfolio against wealth protection strategies.</p>
        </div>
    );
    ...
    const renderResults = () => {
        ...
        return (
            <div className="max-w-3xl mx-auto py-8 text-center">
                <div className="inline-flex items-center gap-2 rounded-full bg-emerald-500/10 px-4 py-1.5 text-sm font-semibold text-emerald-600 mb-8">
                    <CheckCircle2 className="h-4 w-4" />
                    Score Calculation Complete
                </div>

                <h2 className="text-3xl font-serif font-bold text-[#F6F4EF] mb-2">
                    Your Result: <span className="text-[#D4A94E]">{result.scoreLabel}</span>
                </h2>

                <p className="text-lg text-[#D0CCC4] mb-8 max-w-2xl mx-auto">
                ...
                <div className="bg-[#161828] border border-[#2A2D42] rounded-2xl p-8 mb-8 text-left relative overflow-hidden shadow-lg">
                    ...
                    <div className="flex flex-col md:flex-row gap-8 items-start">
                        {/* Score Circle */}
                        <div className="w-full md:w-auto flex flex-col items-center justify-center p-6 bg-[#0C0D18] rounded-xl min-w-[150px] border border-[#2A2D42]">
                            <span className="text-4xl font-bold text-[#F6F4EF] font-serif">
                                {result.score}<span className="text-lg text-[#A8A39A]">/100</span>
                            </span>
                            <span className="text-xs uppercase tracking-wider font-semibold text-[#D4A94E] mt-1">Wealth Score</span>
                        </div>

                        {/* Content */}
                        <div className="flex-1">
                            <h3 className="text-xl font-bold text-[#F6F4EF] mb-2 font-serif">Recommended Strategy: {result.match}</h3>
                            <p className="text-[#D0CCC4] leading-relaxed mb-6">{result.description}</p>

                            <ul className="space-y-3 mb-8">
                                {result.benefits.map((benefit, i) => (
                                    <li key={i} className="flex items-center gap-3 text-sm font-medium text-[#F6F4EF]">
                            ...
                            <div className="flex flex-col sm:flex-row gap-4">
                                ...
                                <div className="text-center sm:text-left px-2">
                                    <p className="text-xs text-[#A8A39A] mt-2">100% Free • No Obligation • Educational Only</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    return (
        <Container className="max-w-4xl min-h-[600px] flex flex-col justify-center">
            ...
            {currentStepIndex > 1 && currentStep.id !== "analyzing" && currentStep.id !== "results" && (
                <button
                    onClick={handleBack}
                    className="self-start flex items-center gap-1 text-sm text-[#A8A39A] hover:text-[#F6F4EF] mb-4 transition-colors"
                >
