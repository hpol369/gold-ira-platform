...
export function QuizWizard() {
    ...
    const renderIntro = () => (
        <div className="text-center space-y-8 py-12">
            <div className="inline-flex items-center gap-2 rounded-full bg-[#B22234]/10 px-4 py-1.5 text-sm font-semibold text-[#B22234] mb-4">
                <AlertTriangle className="h-4 w-4" />
                Are You Exposed?
            </div>
            <h2 className="text-3xl md:text-5xl font-serif font-bold text-[#000080]">{steps[0].title}</h2>
            <p className="text-xl text-slate-600 max-w-xl mx-auto leading-relaxed">{steps[0].subtitle}</p>
            ...
            <p className="text-sm text-slate-500">Takes less than 60 seconds. No SSN required.</p>
        </div>
    );

    const renderQuestion = () => (
        <div className="max-w-2xl mx-auto py-8">
            <h2 className="text-2xl md:text-3xl font-serif font-bold text-[#000080] mb-2 text-center">
                {currentStep.question}
            </h2>
            {currentStep.helper && (
                <p className="text-center text-slate-500 mb-8 text-sm">{currentStep.helper}</p>
            )}
            ...
            <div className="grid grid-cols-1 gap-4 mt-8">
                {currentStep.options?.map((option) => (
                    <button
                        ...
                        className="group relative flex items-center justify-between p-6 text-left rounded-xl border-2 border-slate-200 hover:border-[#B22234]/50 hover:bg-slate-50 transition-all duration-200 bg-white"
                    >
                        <span className="text-lg font-medium text-[#000080] group-hover:text-[#B22234]">
                            {option.label}
                        </span>
                        <div className="h-6 w-6 rounded-full border-2 border-slate-200 group-hover:border-[#B22234] flex items-center justify-center">
                            <div className="h-3 w-3 rounded-full bg-[#B22234] opacity-0 group-hover:opacity-100 transition-opacity" />
                        </div>
                    </button>
                ))}
            </div>
    ...
    const renderAnalyzing = () => (
        <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="w-16 h-16 border-4 border-slate-200 border-t-[#B22234] rounded-full animate-spin mb-8" />
            <h3 className="text-2xl font-serif font-bold text-[#000080] mb-2">Analyzing your profile...</h3>
            <p className="text-slate-500">Comparing your portfolio against wealth protection strategies.</p>
        </div>
    );
    ...
    const renderResults = () => {
        ...
        return (
            <div className="max-w-3xl mx-auto py-8 text-center">
                ...
                <h2 className="text-3xl font-serif font-bold text-[#000080] mb-2">
                    Your Result: <span className="text-[#B22234]">{result.scoreLabel}</span>
                </h2>
                ...
                <div className="bg-white border border-slate-200 rounded-2xl p-8 mb-8 text-left relative overflow-hidden shadow-lg">
                    {/* Top Banner */}
                    <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-[#B22234] to-[#8b1c2a]" />

                    <div className="flex flex-col md:flex-row gap-8 items-start">
                        {/* Score Circle */}
                        <div className="w-full md:w-auto flex flex-col items-center justify-center p-6 bg-slate-50 rounded-xl min-w-[150px] border border-slate-200">
                            <span className="text-4xl font-bold text-[#000080] font-serif">
                                {result.score}<span className="text-lg text-slate-400">/100</span>
                            </span>
                            <span className="text-xs uppercase tracking-wider font-semibold text-[#B22234] mt-1">Wealth Score</span>
                        </div>

                        {/* Content */}
                        <div className="flex-1">
                            <h3 className="text-xl font-bold text-[#000080] mb-2 font-serif">Recommended Strategy: {result.match}</h3>
                            <p className="text-slate-600 leading-relaxed mb-6">{result.description}</p>

                            <ul className="space-y-3 mb-8">
                                {result.benefits.map((benefit, i) => (
                                    <li key={i} className="flex items-center gap-3 text-sm font-medium text-[#000080]">
    ...
    return (
        <Container className="max-w-4xl min-h-[600px] flex flex-col justify-center">
            ...
            {currentStepIndex > 1 && currentStep.id !== "analyzing" && currentStep.id !== "results" && (
                <button
                    onClick={handleBack}
                    className="self-start flex items-center gap-1 text-sm text-slate-500 hover:text-[#000080] mb-4 transition-colors"
                >
