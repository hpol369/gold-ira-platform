...
export function ProviderCard({ provider, variant = "default" }: ProviderCardProps) {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="p-3 bg-slate-700 rounded-xl group-hover:bg-amber-500/20 transition-colors border border-white/10">
            <Building2 className="h-6 w-6 text-slate-400 group-hover:text-amber-400" />
          </div>
          <div>
            ...
            <span className={cn(
              ...
              provider.type === "brokerage" && "bg-blue-500/20 text-blue-400 border-blue-500/30",
              provider.type === "employer" && "bg-green-500/20 text-green-400 border-green-500/30",
              provider.type === "government" && "bg-purple-500/20 text-purple-400 border-purple-500/30"
            )}>
      ...
      <div className="flex items-center text-amber-400 font-semibold text-sm group-hover:gap-2 transition-all">
        View Rollover Guide
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <span className={cn(
          ...
          accountType.taxTreatment === "pre-tax" && "bg-blue-500/20 text-blue-400 border-blue-500/30",
          accountType.taxTreatment === "post-tax" && "bg-green-500/20 text-green-400 border-green-500/30",
          accountType.taxTreatment === "both" && "bg-purple-500/20 text-purple-400 border-purple-500/30"
        )}>
