...
export function ProviderCard({ provider, variant = "default" }: ProviderCardProps) {
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-[#A8A39A]">{provider.typicalTimeline}</div>
          </div>
        </div>
        <ArrowRight className="h-4 w-4 text-[#A8A39A] group-hover:text-amber-700 group-hover:translate-x-1 transition-all" />
      </Link>
    );
  }
  ...
  return (
    <Link
      href={`/rollover/${provider.slug}`}
      className="group block bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-6 hover:border-amber-500/30 hover:bg-[#1E2134]/10 transition-all"
    >
      ...
      <p className="text-sm text-[#A8A39A] mb-4 line-clamp-2">
        {provider.description}
      </p>

      <div className="flex flex-wrap gap-4 text-sm text-[#A8A39A] mb-4">
