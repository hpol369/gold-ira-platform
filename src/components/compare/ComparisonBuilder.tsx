...
export function ComparisonBuilder({ companies }: ComparisonBuilderProps) {
  ...
  return (
    <div className="mt-12 bg-slate-50 rounded-2xl p-8 border border-slate-200">
      <h3 className="text-xl font-bold text-slate-900 mb-4">Build Your Own Comparison</h3>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        ...
        <button
          ...
          className="w-full p-3 bg-amber-500 text-white font-bold rounded-lg hover:bg-amber-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Compare
        </button>
      </div>
      {companyA && companyB && companyA === companyB && (
        <p className="text-red-500 text-sm mt-2">Please select two different companies</p>
      )}
    </div>
  );
}
