...
export default function SilverPriceHubPage() {
    return (
        <main className="min-h-screen bg-white pb-24">
            ...
            <header className="bg-white py-16 border-b border-[#2A2D42] relative overflow-hidden">
                ...
                <Container className="relative z-10">
                    <div className="max-w-4xl mx-auto">
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-8">
                            ...
                            {/* Live Price Card */}
                            <div className="bg-[#0C0D18] backdrop-blur-sm rounded-2xl p-6 border border-[#2A2D42] min-w-[280px]">
                                <div className="text-sm text-[#D0CCC4] mb-2">Current Silver Price</div>
                                <div className="flex items-baseline gap-3 mb-3">
                                    <span className="text-4xl font-bold text-[#F6F4EF]">${currentSilverPrice.toFixed(2)}</span>
                                    <span className="text-lg text-[#D0CCC4]">/oz</span>
                                </div>
                                <div className={`flex items-center gap-2 ${priceChange24h >= 0 ? 'text-green-700' : 'text-red-700'}`}>
                                    ...
                                    <span className="text-[#D0CCC4] text-sm">24h</span>
                                </div>
                                <div className="mt-4 pt-4 border-t border-[#2A2D42]">
                                    <div className="text-xs text-[#D0CCC4]">Gold/Silver Ratio</div>
                                    <div className="text-lg font-bold text-[#D4A94E]">80.5:1</div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <Container className="py-12">
                ...
                <section className="mb-16">
                    <h2 className="text-2xl font-serif font-bold text-[#F6F4EF] mb-6">2026 Expert Predictions At-A-Glance</h2>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse bg-[#0C0D18] backdrop-blur-sm rounded-xl shadow-sm border border-[#2A2D42]">
                            <thead className="bg-[#0C0D18]">
                                <tr>
                                    <th className="p-4 font-bold text-[#F6F4EF] border-b border-[#2A2D42]">Source</th>
                                    <th className="p-4 font-bold text-[#F6F4EF] border-b border-[#2A2D42]">Price Target</th>
                                    <th className="p-4 font-bold text-[#F6F4EF] border-b border-[#2A2D42]">Timeframe</th>
                                    <th className="p-4 font-bold text-[#F6F4EF] border-b border-[#2A2D42]">Stance</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-[#2A2D42] text-sm">
                                {expertPredictions.map((pred, idx) => (
                                    <tr key={idx} className={idx % 2 === 1 ? 'bg-[#121423]' : ''}>
                                        <td className="p-4 text-[#F6F4EF] font-medium">{pred.source}</td>
                    ...
                    <p className="text-sm text-[#D0CCC4] mt-4">* Predictions are compiled from public statements and reports. Past predictions are not indicative of future results.</p>
                </section>

                {/* Prediction Pages Grid */}
                <section className="mb-16">
                    <h2 className="text-2xl font-serif font-bold text-[#F6F4EF] mb-6">Silver Price Analysis Hub</h2>
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {predictionPages.map((page) => (
                            <Link
                                key={page.href}
                                href={page.href}
                                className="group bg-[#0C0D18] backdrop-blur-sm rounded-xl p-6 border border-[#2A2D42] hover:border-gray-500/50 transition-all duration-300 hover:bg-[#1E2134]"
                            >
                                <div className="flex items-start justify-between mb-4">
                                    <div className="p-3 bg-gray-500/20 rounded-lg text-[#A8A39A] group-hover:bg-gray-500/30 transition-colors">
                                ...
                                <h3 className="text-lg font-bold text-[#F6F4EF] mb-2 group-hover:text-[#A8A39A] transition-colors">
                                    {page.title}
                                </h3>
                                <p className="text-sm text-[#D0CCC4] mb-4">
                                    {page.description}
                                </p>
                                <div className="flex items-center text-[#A8A39A] text-sm font-semibold">
                    ...
                    <div className="bg-gradient-to-br from-gray-600/20 to-slate-800/50 rounded-2xl p-8 border border-gray-500/20">
                        <div className="flex items-center gap-4 mb-6">
                            <div className="p-3 bg-gray-500/20 rounded-lg">
                                <DollarSign className="w-8 h-8 text-[#A8A39A]" />
                            </div>
                            <div>
                                <h2 className="text-2xl font-serif font-bold text-[#F6F4EF]">Why Silver Could Be the Investment of the Decade</h2>
                                <p className="text-[#D0CCC4]">What the mainstream financial media won&apos;t tell you</p>
                            </div>
                        </div>
                        ...
                        <div className="grid md:grid-cols-2 gap-6 mb-8">
                            <div className="space-y-4">
                                <div className="flex items-start gap-3">
                                    ...
                                    <p className="text-[#D0CCC4]"><strong className="text-[#F6F4EF]">Industrial Demand Explosion:</strong> Solar panels, EVs, 5G, and electronics require massive silver inputs. Supply can&apos;t keep up.</p>
                                </div>
                                <div className="flex items-start gap-3">
                                    <div className="w-2 h-2 rounded-full bg-gray-400 mt-2"></div>
                                    <p className="text-[#D0CCC4]"><strong className="text-[#F6F4EF]">Gold/Silver Ratio at Extremes:</strong> Currently 80:1 vs historical average of 15:1. Silver has massive catch-up potential.</p>
                                </div>
                                <div className="flex items-start gap-3">
                                    <div className="w-2 h-2 rounded-full bg-gray-400 mt-2"></div>
                                    <p className="text-[#D0CCC4]"><strong className="text-[#F6F4EF]">Supply Deficit Growing:</strong> Mine production has peaked while demand accelerates. Above-ground stockpiles are depleting.</p>
                                </div>
                            </div>
                            <div className="space-y-4">
                                <div className="flex items-start gap-3">
                                    <div className="w-2 h-2 rounded-full bg-gray-400 mt-2"></div>
                                    <p className="text-[#D0CCC4]"><strong className="text-[#F6F4EF]">Monetary Insurance:</strong> Like gold, silver is real money that can&apos;t be printed. Central banks are losing credibility.</p>
                                </div>
                                <div className="flex items-start gap-3">
                                    <div className="w-2 h-2 rounded-full bg-gray-400 mt-2"></div>
                                    <p className="text-[#D0CCC4]"><strong className="text-[#F6F4EF]">Tiny Market Size:</strong> The entire silver market is ~$30B. A small shift from bonds/stocks could send prices parabolic.</p>
                                </div>
                                <div className="flex items-start gap-3">
                                    <div className="w-2 h-2 rounded-full bg-gray-400 mt-2"></div>
                                    <p className="text-[#D0CCC4]"><strong className="text-[#F6F4EF]">Inflation Hedge:</strong> With real rates negative and debt at record highs, hard assets protect purchasing power.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                            ...
                            <Button variant="outline" size="lg" asChild className="border-white/30 text-[#F6F4EF] hover:bg-[#1E2134]">
