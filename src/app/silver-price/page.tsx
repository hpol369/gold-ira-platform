...
export default function SilverPriceHubPage() {
    return (
        <main className="min-h-screen bg-slate-900 pb-24">
            ...
            <header className="bg-gradient-to-br from-slate-800 to-slate-900 py-16 border-b border-white/10 relative overflow-hidden">
                ...
                <Container className="relative z-10">
                    <div className="max-w-4xl mx-auto">
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-8">
                            ...
                            {/* Live Price Card */}
                            <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10 min-w-[280px]">
                                <div className="text-sm text-slate-400 mb-2">Current Silver Price</div>
                                <div className="flex items-baseline gap-3 mb-3">
                                    <span className="text-4xl font-bold text-white">${currentSilverPrice.toFixed(2)}</span>
                                    <span className="text-lg text-slate-400">/oz</span>
                                </div>
                                ...
                                <div className="mt-4 pt-4 border-t border-white/10">
                                    <div className="text-xs text-slate-500">Gold/Silver Ratio</div>
                                    <div className="text-lg font-bold text-amber-400">80.5:1</div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <Container className="py-12">
                ...
                <section className="mb-16">
                    <h2 className="text-2xl font-serif font-bold text-white mb-6">2026 Expert Predictions At-A-Glance</h2>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse bg-white/5 backdrop-blur-sm rounded-xl shadow-sm border border-white/10">
                            <thead className="bg-slate-800/50">
                                <tr>
                                    <th className="p-4 font-bold text-white border-b border-white/10">Source</th>
                                    <th className="p-4 font-bold text-white border-b border-white/10">Price Target</th>
                                    <th className="p-4 font-bold text-white border-b border-white/10">Timeframe</th>
                                    <th className="p-4 font-bold text-white border-b border-white/10">Stance</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-white/10 text-sm">
                                {expertPredictions.map((pred, idx) => (
                                    <tr key={idx} className={idx % 2 === 1 ? 'bg-slate-800/30' : ''}>
                                        <td className="p-4 text-white font-medium">{pred.source}</td>
                                        <td className="p-4 text-green-400 font-bold text-lg">{pred.target}</td>
                                        <td className="p-4 text-slate-400">{pred.timeframe}</td>
                                        <td className="p-4">
                    ...
                    <p className="text-sm text-slate-500 mt-4">* Predictions are compiled from public statements and reports. Past predictions are not indicative of future results.</p>
                </section>

                {/* Prediction Pages Grid */}
                <section className="mb-16">
                    <h2 className="text-2xl font-serif font-bold text-white mb-6">Silver Price Analysis Hub</h2>
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {predictionPages.map((page) => (
                            <Link
                                key={page.href}
                                href={page.href}
                                className="group bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10 hover:border-gray-500/50 transition-all duration-300 hover:bg-white/10"
                            >
                                <div className="flex items-start justify-between mb-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-xs text-slate-500">{page.volume} searches/mo</span>
                                    </div>
                                </div>
                ...
                <section className="mb-16">
                    <div className="bg-gradient-to-br from-gray-600/20 to-slate-800/50 rounded-2xl p-8 border border-gray-500/20">
                        <div className="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                <h2 className="text-2xl font-serif font-bold text-white">Why Silver Could Be the Investment of the Decade</h2>
                                <p className="text-slate-400">What the mainstream financial media won&apos;t tell you</p>
                            </div>
                        </div>
                        ...
                        <div className="grid md:grid-cols-2 gap-6 mb-8">
                            <div className="space-y-4">
                                <div className="flex items-start gap-3">
                                    ...
                                    <p className="text-slate-300"><strong className="text-white">Industrial Demand Explosion:</strong> Solar panels, EVs, 5G, and electronics require massive silver inputs. Supply can&apos;t keep up.</p>
                                </div>
                                <div className="flex items-start gap-3">
                                    <div className="w-2 h-2 rounded-full bg-gray-400 mt-2"></div>
                                    <p className="text-slate-300"><strong className="text-white">Gold/Silver Ratio at Extremes:</strong> Currently 80:1 vs historical average of 15:1. Silver has massive catch-up potential.</p>
                                </div>
                                <div className="flex items-start gap-3">
                                    <div className="w-2 h-2 rounded-full bg-gray-400 mt-2"></div>
                                    <p className="text-slate-300"><strong className="text-white">Supply Deficit Growing:</strong> Mine production has peaked while demand accelerates. Above-ground stockpiles are depleting.</p>
                                </div>
                            </div>
                            <div className="space-y-4">
                                <div className="flex items-start gap-3">
                                    <div className="w-2 h-2 rounded-full bg-gray-400 mt-2"></div>
                                    <p className="text-slate-300"><strong className="text-white">Monetary Insurance:</strong> Like gold, silver is real money that can&apos;t be printed. Central banks are losing credibility.</p>
                                </div>
                                <div className="flex items-start gap-3">
                                    <div className="w-2 h-2 rounded-full bg-gray-400 mt-2"></div>
                                    <p className="text-slate-300"><strong className="text-white">Tiny Market Size:</strong> The entire silver market is ~$30B. A small shift from bonds/stocks could send prices parabolic.</p>
                                </div>
                                <div className="flex items-start gap-3">
                                    <div className="w-2 h-2 rounded-full bg-gray-400 mt-2"></div>
                                    <p className="text-slate-300"><strong className="text-white">Inflation Hedge:</strong> With real rates negative and debt at record highs, hard assets protect purchasing power.</p>
                                </div>
                            </div>
                        </div>
