...
export default function ReviewsPage() {
  ...
  return (
    <main className="min-h-screen flex flex-col bg-[#0C0D18]">
      ...
      <header className="bg-[#0C0D18] border-b border-[#2A2D42] py-16 md:py-20 relative overflow-hidden">
        ...
        <Container className="relative z-10">
          <div className="max-w-3xl">
            ...
            <h1 className="text-4xl md:text-5xl font-serif font-black mb-6 text-[#F6F4EF]">
              Gold IRA Company Reviews
            </h1>
            <p className="text-xl text-[#D0CCC4] leading-relaxed mb-8">
              Picking the wrong Gold IRA company could cost you thousands in hidden fees. We dug through <strong className="text-[#F6F4EF]">BBB complaints</strong>, fee structures, and customer experiences.
            </p>
      ...
      <section className="bg-[rgba(197,149,46,0.1)] text-white py-6 border-y border-blue-900">
        <Container>
          <Link
          ...
          >
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 bg-amber-400 text-[#F6F4EF] px-3 py-1.5 rounded-full text-sm font-bold shadow-lg">
      ...
      <section className="py-16 bg-[#0C0D18]">
        <Container>
          <div className="grid gap-4 max-w-4xl mx-auto">
            {companies.map((company, index) => (
              <Link
                ...
                className={`group bg-[#161828] rounded-xl border p-6 hover:shadow-lg transition-all ${company.featured
                    ? "border-amber-200 hover:border-amber-400 ring-4 ring-amber-50"
                    : "border-[#2A2D42] hover:border-[rgba(197,149,46,0.55)]/30"
                  }`}
              >
                <div className="flex flex-col md:flex-row md:items-center gap-4">
                  ...
                  <div className="flex items-center gap-4 flex-1">
                    <div className={`shrink-0 w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg ${index === 0
                        ? "bg-[rgba(197,149,46,0.1)] text-white"
                        : index < 3
                          ? "bg-[#121423] text-[#D0CCC4] border border-[#2A2D42]"
                          : "bg-[#0C0D18] text-[#A8A39A] border border-[#2A2D42]"
                      }`}>
                      {index + 1}
                    </div>
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <h2 className={`text-xl font-bold transition-colors whitespace-nowrap ${company.featured
                            ? "text-[#F6F4EF] group-hover:text-blue-800"
                            : "text-[#F6F4EF] group-hover:text-[#F6F4EF]"
                          }`}>
                          {company.name}
                        </h2>
                        {company.featured && (
                        ...
                        {company.highlight && !company.featured && (
                          <span className="text-xs px-2 py-1 bg-[#121423] text-[#D0CCC4] rounded-full font-semibold border border-[#2A2D42]">
                            {company.highlight}
                          </span>
                        )}
                      </div>
                      <p className="text-sm text-[#A8A39A] font-medium">{company.bestFor}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-6 text-sm">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-[#D0CCC4]">{company.bbbRating} BBB</span>
                    </div>
                    <div className="text-[#D0CCC4] whitespace-nowrap font-medium">
                      ${company.minInvestment.toLocaleString()} min
                    </div>
                    <ArrowRight className="w-5 h-5 text-[#A8A39A] group-hover:text-[#F6F4EF] group-hover:translate-x-1 transition-all" />
                  </div>
                </div>

                {/* Quick Pros for top companies */}
                {index < 5 && (
                  <div className="mt-4 pt-4 border-t border-[#2A2D42] flex flex-wrap gap-3">
                    {company.pros.slice(0, 3).map((pro, i) => (
                      <div key={i} className="flex items-center gap-1.5 text-xs text-[#D0CCC4] font-medium">
          ...
          <div className="max-w-4xl mx-auto mt-16">
            <h2 className="text-2xl font-bold text-[#F6F4EF] mb-2 flex items-center gap-2 font-serif">
              <FileSearch className="w-6 h-6 text-[#F6F4EF]" />
              Investigations & Complaint Analysis
            </h2>
            <p className="text-[#D0CCC4] mb-6">Deep dives into lawsuits, BBB complaints, and company legitimacy.</p>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-[#A8A39A]">{page.description}</p>
                    </div>
                  </div>
                </Link>
              ))}
          ...
          <div className="max-w-4xl mx-auto mt-16 p-8 bg-[#0C0D18] rounded-2xl border border-[#2A2D42]">
            <h2 className="text-2xl font-serif font-bold text-[#F6F4EF] mb-4">How We Actually Review These Companies</h2>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <strong className="text-[#F6F4EF]">BBB Complaints</strong>
                  <p className="text-sm text-[#A8A39A]">What real customers actually complained about</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle2 className="w-5 h-5 text-green-600 mt-0.5" />
                <div>
                  <strong className="text-[#F6F4EF]">Hidden Fees</strong>
                  <p className="text-sm text-[#A8A39A]">The charges they don&apos;t advertise up front</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle2 className="w-5 h-5 text-green-600 mt-0.5" />
                <div>
                  <strong className="text-[#F6F4EF]">Minimum Investment</strong>
                  <p className="text-sm text-[#A8A39A]">Does it match what real people have to invest?</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <CheckCircle2 className="w-5 h-5 text-green-600 mt-0.5" />
                <div>
                  <strong className="text-[#F6F4EF]">Sales Pressure</strong>
                  <p className="text-sm text-[#A8A39A]">Do they educate, or do they push?</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto mt-12">
            <h3 className="text-lg font-bold text-[#F6F4EF] mb-4">Related Guides</h3>
            <div className="grid md:grid-cols-3 gap-4">
              <Link href="/guide/gold-ira-fees" className="p-4 bg-[#161828] rounded-lg border border-[#2A2D42] hover:border-[rgba(197,149,46,0.55)]/30 hover:shadow-sm transition-all">
                <span className="text-sm font-semibold text-[#F6F4EF]">Gold IRA Fees Guide &rarr;</span>
              </Link>
              <Link href="/learn/gold-ira-scams" className="p-4 bg-[#161828] rounded-lg border border-[#2A2D42] hover:border-[rgba(197,149,46,0.55)]/30 hover:shadow-sm transition-all">
                <span className="text-sm font-semibold text-[#F6F4EF]">Scams to Avoid &rarr;</span>
              </Link>
              <Link href="/compare/gold-ira-vs-401k" className="p-4 bg-[#161828] rounded-lg border border-[#2A2D42] hover:border-[rgba(197,149,46,0.55)]/30 hover:shadow-sm transition-all">
                <span className="text-sm font-semibold text-[#F6F4EF]">Gold IRA vs 401k &rarr;</span>
              </Link>
            </div>
          </div>
        </Container>
