                ...
                <Container className="relative z-10">
                    <div className="max-w-4xl mx-auto">
                        <nav className="flex items-center gap-2 text-sm text-slate-500 mb-6">
                            ...
                            <span>/</span>
                            <Link href="/crypto-ira" className="hover:text-[#B22234]">Crypto IRA</Link>
                            <span>/</span>
                            <span className="text-slate-900">Best Companies</span>
                        </nav>
                            ...
                            <table className="w-full border-collapse">
                                <thead>
                                    <tr className="bg-slate-50">
                                        ...
                                        {companies.map((c) => (
                                            <th key={c.name} className="text-left px-4 py-3 text-slate-900 font-semibold border-b border-slate-200">
                                ...
                                <tbody>
                                    <tr>
                                        <td className="px-4 py-3 border-b border-slate-100 font-medium text-slate-700">Trading Fee</td>
                                        {companies.map((c) => (
                                            <td key={c.name} className="px-4 py-3 border-b border-slate-100 text-slate-600">{c.tradingFee}</td>
                                        ))}
                                    </tr>
                                    <tr className="bg-slate-50/50">
                                        <td className="px-4 py-3 border-b border-slate-100 font-medium text-slate-700">Monthly Fee</td>
                                        {companies.map((c) => (
                                            <td key={c.name} className="px-4 py-3 border-b border-slate-100 text-slate-600">{c.monthlyFee}</td>
                                        ))}
                                    </tr>
                                    <tr>
                                        <td className="px-4 py-3 border-b border-slate-100 font-medium text-slate-700">Minimum Investment</td>
                                        {companies.map((c) => (
                                            <td key={c.name} className="px-4 py-3 border-b border-slate-100 text-slate-600">{c.minimum}</td>
                                        ))}
                                    </tr>
                                    <tr className="bg-slate-50/50">
                                        <td className="px-4 py-3 border-b border-slate-100 font-medium text-slate-700">Crypto Selection</td>
                                        {companies.map((c) => (
                                            <td key={c.name} className="px-4 py-3 border-b border-slate-100 text-slate-600">{c.cryptos} cryptos</td>
                                        ))}
                                    </tr>
                                    <tr>
                                        <td className="px-4 py-3 border-b border-slate-100 font-medium text-slate-700">Other Assets</td>
                                        {companies.map((c) => (
                                            <td key={c.name} className="px-4 py-3 border-b border-slate-100 text-slate-600">{c.otherAssets}</td>
                                        ))}
                                    </tr>
                                    <tr className="bg-slate-50/50">
                                        <td className="px-4 py-3 border-b border-slate-100 font-medium text-slate-700">Insurance</td>
                                        {companies.map((c) => (
                                            <td key={c.name} className="px-4 py-3 border-b border-slate-100 text-slate-600">{c.insurance}</td>
                                        ))}
                                    </tr>
                                    <tr>
                                        <td className="px-4 py-3 border-b border-slate-100 font-medium text-slate-700">Storage</td>
                                        {companies.map((c) => (
                                            <td key={c.name} className="px-4 py-3 border-b border-slate-100 text-slate-600">{c.storage}</td>
                                        ))}
                                    </tr>
                                    <tr className="bg-slate-50/50">
                                        <td className="px-4 py-3 font-medium text-slate-700">Our Rating</td>
                                        {companies.map((c) => (
                                            <td key={c.name} className="px-4 py-3 font-bold text-[#000080]">{c.rating}/5</td>
                                        ))}
                                    </tr>
                                </tbody>
                            </table>
                ...
                <Container>
                    <div className="max-w-4xl mx-auto space-y-12">
                        ...
                        {companies.map((company, idx) => (
                            <div key={company.name} className="bg-white rounded-xl border border-slate-200 p-6 md:p-8">
                                <div className="flex items-start justify-between mb-4">
                                    <div>
                                        <span className="text-xs font-bold text-white bg-[#B22234] px-3 py-1 rounded-full mb-2 inline-block">
                                            #{idx + 1} {company.highlight}
                                        </span>
                                        <h3 className="text-2xl font-bold text-[#000080] mt-2">{company.name}</h3>
                                    </div>
                                    <div className="text-right">
                                        <div className="text-3xl font-bold text-[#000080]">{company.rating}</div>
                    ...
                    <div className="max-w-4xl mx-auto">
                        ...
                        <div className="grid md:grid-cols-3 gap-6 mt-8">
                            <div className="bg-green-50 border border-green-200 rounded-xl p-6">
                                ...
                                <h3 className="font-bold text-slate-900 mb-2">Lowest Fees</h3>
                            ...
                            <div className="bg-blue-50 border border-blue-200 rounded-xl p-6">
                                ...
                                <h3 className="font-bold text-slate-900 mb-2">Most Crypto Options</h3>
                                <p className="text-slate-600 text-sm mb-4">
                            ...
                            <div className="bg-amber-50 border border-amber-200 rounded-xl p-6">
                                ...
                                <h3 className="font-bold text-slate-900 mb-2">Zero Trade Fees</h3>
                                <p className="text-slate-600 text-sm mb-4">
                        ...
                        <div className="bg-white rounded-xl border border-slate-200 p-6 mt-6">
                            <ul className="space-y-4">
                                <li className="flex items-start gap-3">
                                    ...
                                    <div>
                                        <strong className="text-slate-900">Cold Storage:</strong>{" "}
                                        <span className="text-slate-600">All three providers keep crypto in offline wallets (iTrustCapital uses Fireblocks, Alto uses Coinbase Custody, Bitcoin IRA uses BitGo).</span>
                                    </div>
                                </li>
                                <li className="flex items-start gap-3">
                                    <Shield className="w-5 h-5 text-green-500 mt-1 flex-shrink-0" />
                                    <div>
                                        <strong className="text-slate-900">Insurance Coverage:</strong>{" "}
                                        <span className="text-slate-600">iTrustCapital and Bitcoin IRA carry $250 million in insurance through BitGo. Alto IRA is backed by Coinbase&apos;s institutional insurance.</span>
                                    </div>
                                </li>
                                <li className="flex items-start gap-3">
                                    <Shield className="w-5 h-5 text-green-500 mt-1 flex-shrink-0" />
                                    <div>
                                        <strong className="text-slate-900">Regulated Custodians:</strong>{" "}
                                        <span className="text-slate-600">All providers use IRS-approved custodians and are subject to regulatory oversight. Your IRA is a legally distinct account from the company itself.</span>
                                    </div>
                                </li>
                            </ul>
                        </div>
