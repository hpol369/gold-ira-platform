      ...
      <section className="relative pt-24 pb-12 overflow-hidden bg-[#0C0D18]">
        ...
        <Container className="relative z-10">
          <nav className="flex items-center gap-2 text-sm text-[#A8A39A] mb-6">
            ...
            <Link href="/grandchildren" className="hover:text-[#F6F4EF]">Grandchildren Legacy</Link>
            <ChevronRight className="h-4 w-4" />
                ...
                <div className="mb-12">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    {article.ageConsiderations.map((age, i) => (
                      <div key={i} className="bg-[#0C0D18] border border-[#2A2D42] rounded-xl p-5">
                        <h3 className="font-bold text-green-700 mb-3">{age.ageRange}</h3>
                        <ul className="space-y-2">
                          {age.tips.map((tip, j) => (
                            <li key={j} className="flex items-start gap-2 text-sm text-[#D0CCC4]">
                  ...
                  <div className="space-y-3">
                    {article.taxImplications.map((tax, i) => (
                      <div key={i} className="text-sm border-b border-amber-200 pb-3 last:border-0 last:pb-0">
                        <span className="text-[#F6F4EF] font-medium">{tax.scenario}:</span>{" "}
                        <span className="text-[#D0CCC4]">{tax.taxTreatment}</span>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    {relatedArticles.map((related) => (
                      <Link key={related.slug} href={`/grandchildren/${related.slug}`}
                        className="bg-[#0C0D18] hover:bg-[#1E2134] border border-[#2A2D42] hover:border-green-300 rounded-lg p-4 transition-all group">
                        <h3 className="font-medium text-[#F6F4EF] group-hover:text-green-600">{related.title}</h3>
