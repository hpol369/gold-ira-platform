          ...
          <ol className="space-y-3 mb-4">
            {section.numberedList.map((item, i) => (
              <li key={i} className="flex items-start gap-3 text-sm">
                <span className="bg-green-500/20 text-green-400 w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold">
                  {i + 1}
                </span>
                <span className="text-slate-300" dangerouslySetInnerHTML={{
      ...
      <section className="relative pt-24 pb-12 overflow-hidden">
        ...
        <Container className="relative z-10">
          <nav className="flex items-center gap-2 text-sm text-slate-400 mb-6">
            ...
            <Link href="/grandchildren" className="hover:text-white">Grandchildren Legacy</Link>
            <ChevronRight className="h-4 w-4" />
          ...
          <div className="lg:grid lg:grid-cols-[280px_1fr] lg:gap-12">
            ...
            <article>
              ...
              {article.ageConsiderations && article.ageConsiderations.length > 0 && (
                <div className="mb-12">
                  <h2 className="text-2xl font-serif font-bold text-white mb-6 flex items-center gap-3">
                    <Users className="h-6 w-6 text-green-400" /> Age Considerations
                  </h2>
                  <div className="grid md:grid-cols-2 gap-4">
                    {article.ageConsiderations.map((age, i) => (
                      <div key={i} className="bg-white/5 border border-white/10 rounded-xl p-5">
                        <h3 className="font-bold text-green-400 mb-3">{age.ageRange}</h3>
              ...
              {article.taxImplications && article.taxImplications.length > 0 && (
                <div className="mb-12 bg-amber-500/10 border border-amber-500/30 rounded-xl p-6">
                  ...
                  <div className="space-y-3">
                    {article.taxImplications.map((tax, i) => (
                      <div key={i} className="text-sm border-b border-amber-500/20 pb-3 last:border-0 last:pb-0">
                        <span className="text-white font-medium">{tax.scenario}:</span>{" "}
                        <span className="text-slate-300">{tax.taxTreatment}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {article.faqs.length > 0 && (
                <section className="my-12">
                  <h2 className="text-2xl font-serif font-bold text-white mb-6 flex items-center gap-3">
                    <HelpCircle className="h-6 w-6 text-green-400" /> Frequently Asked Questions
                  </h2>
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    {relatedArticles.map((related) => (
                      <Link key={related.slug} href={`/grandchildren/${related.slug}`}
                        className="bg-white/5 hover:bg-white/10 border border-white/10 hover:border-green-500/30 rounded-lg p-4 transition-all group">
                        <h3 className="font-medium text-white group-hover:text-green-400">{related.title}</h3>
