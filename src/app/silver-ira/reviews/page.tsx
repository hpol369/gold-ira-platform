...
export default function SilverIraReviewsPage() {
    return (
        <main className="min-h-screen bg-[#0C0D18]">
            ...
            <section className="py-12 bg-[#0C0D18]">
                <Container>
                    <div className="max-w-4xl mx-auto">
                        <h2 className="text-xl font-bold text-[#F6F4EF] text-center mb-6">How We Rate Silver IRA Companies</h2>
                        <div className="flex flex-wrap justify-center gap-4">
                            {ratingCategories.map((category, index) => (
                                <div key={index} className="bg-[#0C0D18] border border-[#2A2D42] rounded-lg px-4 py-2">
                                    <span className="text-sm text-[#F6F4EF] font-semibold">{category.name}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="max-w-4xl mx-auto space-y-12">
                        {companyReviews.map((company, index) => (
                            ...
                            >
                                    ...
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                        <div>
                                            <div className="flex items-center gap-3 mb-2">
                                                {company.featured && (
                                                    <span className="px-2 py-1 bg-[#DC2626] text-[#F6F4EF] text-xs font-bold rounded">
                                                        EDITOR'S CHOICE
                                                    </span>
                                                )}
                                            ...
                                            <h3 className="text-2xl font-bold text-[#F6F4EF]">{company.name}</h3>
                                            <p className="text-sm text-[#D0CCC4] mt-1">
                                                Est. {company.yearFounded} | {company.reviewCount.toLocaleString()} verified reviews
                                            </p>
                                        </div>
                                        <div className="text-center">
                                            <div className="text-4xl font-bold text-[#D4A94E]">{company.overallRating}</div>
                                            <div className="flex justify-center mt-1">
                                                {[1, 2, 3, 4, 5].map((star) => (
                                                    <Star
                                                        key={star}
                                                        className={`h-5 w-5 ${star <= Math.floor(company.overallRating) ? 'text-[#D4A94E] fill-[#B22234]' : 'text-[#D0CCC4]'}`}
                                                    />
                                                ))}
                                            </div>
                                            <p className="text-xs text-[#D0CCC4] mt-1">Overall Rating</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="p-6">
                                    {/* Verdict */}
                                    <div className="bg-[#0C0D18] rounded-lg p-4 mb-6 border-l-4 border-amber-500">
                                        <p className="text-[#F6F4EF] font-medium">{company.verdict}</p>
                                    </div>

                                    {/* Quick Stats */}
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                                        <div className="bg-[#0C0D18] rounded-lg p-3 border border-[#2A2D42] text-center">
                                        ...
                                        <div className="bg-[#0C0D18] rounded-lg p-3 border border-[#2A2D42] text-center">
                                            <div className="text-xs text-[#D0CCC4] mb-1">BBB Rating</div>
                                            <div className="font-bold text-green-700">{company.bbbRating}</div>
                                        </div>
                                        <div className="bg-[#0C0D18] rounded-lg p-3 border border-[#2A2D42] text-center">
                                            <div className="text-xs text-[#D0CCC4] mb-1">Trustpilot</div>
                                            <div className="font-bold text-[#F6F4EF]">{company.trustpilotRating}/5</div>
                                        </div>
                                    </div>

                                    {/* Category Ratings */}
                                    <div className="mb-6">
                                        <h4 className="font-semibold text-[#F6F4EF] mb-3">Category Ratings</h4>
                                        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                                            {Object.entries(company.ratings).map(([key, value]) => (
                                                <div key={key} className="bg-[#0C0D18] rounded-lg p-3 text-center">
                                                    <div className="text-xs text-[#D0CCC4] mb-1 capitalize">
                                                        {key.replace(/([A-Z])/g, ' $1').trim()}
                                                    </div>
                                                    <div className="text-lg font-bold text-[#F6F4EF]">{value}</div>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                        ...
                                        <div className="bg-[rgba(220,38,38,0.08)]0/10 p-4 rounded-xl border border-red-500/20">
                                            ...
                                            <ul className="space-y-2">
                                                {company.cons.map((con, idx) => (
                                                    <li key={idx} className="text-sm text-[#D0CCC4]">
                                                        * {con}
                                                    </li>
                                                ))}
                                            </ul>
                                    ...
                                    {/* Customer Quotes */}
                                    <div className="mb-6">
                                        <h4 className="font-semibold text-[#F6F4EF] mb-3">What Customers Say</h4>
                                        <div className="grid md:grid-cols-2 gap-4">
                                            {company.customerQuotes.map((quote, idx) => (
                                                <div key={idx} className="bg-[#0C0D18] rounded-lg p-4 border border-white/5">
                                                    <p className="text-[#D0CCC4] text-sm italic mb-2">"{quote.quote}"</p>
                                                    <p className="text-xs text-[#D0CCC4]">- {quote.author}</p>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-4">
                                        <Button className="bg-[#DC2626] hover:bg-[#991B1B] text-white flex-1" size="lg" asChild>
                                        ...
                                        <Button variant="outline" size="lg" className="flex-1 border-[#3F4460] text-[#D0CCC4] hover:bg-[#1E2134]" asChild>
            ...
            <section className="py-16 bg-[#0C0D18]">
                <Container>
                    <div className="max-w-3xl mx-auto text-center bg-[#161828] rounded-2xl p-8 border border-[#2A2D42]">
                        <h2 className="text-2xl font-bold text-[#F6F4EF] mb-4">Not Sure Which Company Is Right for You?</h2>
            ...
            <section className="py-16 bg-[#0C0D18]">
                <Container>
                    <h2 className="text-2xl font-bold text-[#F6F4EF] text-center mb-8">More Silver IRA Resources</h2>
                    <div className="grid md:grid-cols-3 gap-6">
                        <Link href="/silver-ira/companies" className="group bg-[#0C0D18] border border-[#2A2D42] rounded-xl p-6 hover:bg-[#1E2134] transition-colors">
                            ...
                            <p className="text-sm text-[#D0CCC4]">Compare top providers side by side.</p>
                        </Link>
                        <Link href="/silver-ira/best-company" className="group bg-[#0C0D18] border border-[#2A2D42] rounded-xl p-6 hover:bg-[#1E2134] transition-colors">
                            <h3 className="font-bold text-[#F6F4EF] mb-2 group-hover:text-[#D4A94E] transition-colors">Best Silver IRA Company</h3>
                            <p className="text-sm text-[#D0CCC4]">Our top pick for silver IRA investing.</p>
                        </Link>
